...
def scrape_ticker(context, ticker):
    ...
    try:
        # --- 1. CAPTURE 7D CHART ---
        page.goto(chart_url, wait_until="load", timeout=45000)
        # Wait specifically for the chart container to be visible
        ...
        if chart_element:
            img_bytes = chart_element.screenshot()
            chart_base64 = base64.b64encode(img_bytes).decode('utf-8')

        # --- 2. CAPTURE 30D HEATMAP DATA ---
        page.goto(data_url, wait_until="load", timeout=45000)
        # Wait for TH to ensure the Date/Header column is loaded
        ...
        for row in rows:
            # Captures th AND td to ensure Date column isn't missed
        ...
        requests.post(SHEETS_BRIDGE_URL, json=payload, timeout=20)
        print(f"[{ticker}] Done.")
